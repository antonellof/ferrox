...
mod tests {
    ...
    fn the_fused_attention_tail_matches_the_host_tail() {
        use crate::fused_layer::LayerFfnParts;
        use ferrox_core::weight_matrix::hadamard::{FoldSite, HadamardFold};
        use ferrox_moe::{ExpertWeights, GluAct};
        ...
        let mut host: Vec<f32> = residual
            ...
            .map(|(a, b)| a + b)
            .collect();
        let normed = ferrox_core::rms_norm(&host, &ffn_norm, eps);
        ...
        let device = ferrox_metal::gdn_branch::launch_attn_tail(
            &out_proj,
            Some(&plan),
            &launches.as_metal(),
            &branch,
