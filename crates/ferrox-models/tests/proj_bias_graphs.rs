...
//! path. `ferrox_models::proj_bias` is that slot -- `AttnWeights::
...
//! | `llama_biases` | the OPTIONAL case: `llama.cpp`'s own graph creates `attn_output.bias` and all three FFN biases `TENSOR_NOT_REQUIRED` and applies them when present, on a GATED SwiGLU with RMSNorm -- the one file that exercises `ffn_gate.bias`; ferrox used to refuse it as carrying unread tensors |
//!
...
//!     crates/ferrox-models/tests/fixtures/starcoder2_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_proj_bias_fixture.py codeshell \
//!     crates/ferrox-models/tests/fixtures/codeshell_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_proj_bias_fixture.py jais2 \
//!     crates/ferrox-models/tests/fixtures/jais2_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_proj_bias_fixture.py llama \
//!     crates/ferrox-models/tests/fixtures/llama_biases_tiny.gguf
...
/// reason was measured the way theirs was: with ferrox's `gelu` made to
