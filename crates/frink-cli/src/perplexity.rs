//! `frink perplexity` — corpus perplexity, llama.cpp's `perplexity` tool.
//!
//! # Why this exists
//!
//! Every quality claim in this repo was, until this command, either a
//! token-for-token comparison (`frink verify`) or a single-position
//! distribution comparison (`frink parity`). Neither answers the
...
//!    noise `frink parity` already calls a match.
...
//! 4. **Tokenization is frink's.** Upstream tokenizes with libllama.
//!    The evidence that these agree is `frink parity`'s tokenizer half,
...
/// makes a frink number comparable to a published one without both
...
pub fn run(args: PerplexityArgs) -> anyhow::Result<()> {
    ...
    // Loading and tokenizing go through the same helper `frink verify`
    // and `frink parity` use, so the tokenizer this number is computed
