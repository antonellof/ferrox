...
//! "simplifying" it away.
//!
//! Both are read here; both are written by `frink imatrix`. The reader
...
/// What `frink quantize --imatrix` consumes: per weight name, one
...
fn read_gguf(path: &Path) -> Result<ImatrixWeights> {
    ...
    // (`quantize.cpp:238-246`), so frink does too: a hand-edited file
...
/// How `frink imatrix` spells its output. Same two words as
...
/// stack's are 2-D. Reproducing that keeps a frink file's headers
...
mod tests {
    ...
    /// The GGUF round trip: what `frink quantize` reads back is
