//! `frink bench --suite` / `--render`: the llama-bench-shaped ledger.
//!
//! Numbers are `pp<N>` / `tg<N>` from `frink bench` vs `llama-bench`,
...
pub fn run_suite(args: SuiteArgs) -> anyhow::Result<()> {
    ...
    // stray `frink` process from an earlier run held the instance
    ...
    if measured == 0 {
        eprintln!(
            "frink bench: no entry produced a measurement, so {} was left alone. \
