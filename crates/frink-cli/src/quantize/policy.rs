//! Two decisions `frink quantize` has to make, and the refusals that
//! follow from them:
//!
//! 1. **Which target may be written at all.** frink READS every quant
...
//!    repo refuses, so a target frink cannot encode is refused BY
...
//!    `--pure` said to write the uniform file instead, because frink
...
/// The quantization targets `frink quantize` can actually encode.
///
...
// name and would be one more place where frink's word for a quant and
...
impl Target {
    ...
    /// so a file frink writes reports its mix the way every other tool
    ...
    pub fn fallback_note(self) -> &'static str {
        match self {
            ...
            // Q6_K -> Q8_0 -> F16. frink HAS a Q8_0 encoder, and
...
/// frink encoder that writes it -- `None` where there is none.
///
...
/// frink target and records the same `general.file_type`.
///
...
/// frink, and a typo.
///
/// A third arm used to live here, `MixNeedsPure`: frink could encode
...
pub enum TargetRefusal {
    /// A real llama.cpp target that frink has no encoder for.
...
/// What `frink quantize` can write, in the form a user has to type it.
...
/// being checked. The reason string is not decoration -- `frink
...
/// 1]` tensor is 1-D to llama.cpp and must be to frink too, or the two
...
/// What `frink quantize` does with one tensor, given the type the mix
...
mod tests {
    ...
    /// types a quant frink has no encoder for must be told so -- not
    ...
    fn a_target_with_no_encoder_is_refused_by_name_and_says_what_can_be_written() {
        ...
        // And it must say frink STOPS, not that it did something
    ...
    /// need `--pure`, because frink could encode Q4_K blocks and not
    ...
    /// Every llama.cpp target frink has no encoder for refuses, and
    ...
    /// table is a target `frink quantize --type` can never reach, so
