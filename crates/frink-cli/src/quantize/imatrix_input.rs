//! The consumer side of an importance matrix in `frink quantize`:
...
//! are `crate::imatrix::file`, shared with `frink imatrix`, so the
...
mod tests {
    ...
    /// frink file match a llama.cpp file quantized from the same
