...
///
/// These four are every condition the five mixes frink can write
...
/// `llama_tensor_get_type`'s else-if chain for the five mixes frink
...
/// to_types_frink_can_encode` is what stops a row from naming a type
/// with no encoder.
///
...
impl Recipe {
    ...
    pub fn tensor_type(&mut self, name: &str, shape: &[u64]) -> GgmlType {
        ...
        // back to a DIFFERENT type. frink refuses that case in `plan`
