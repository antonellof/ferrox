...
//! its three output files with frink's own GGUF reader. A transcription
...
//! `models/Llama-3.2-1B-Instruct-Q8_0.gguf` through frink's reader and
...
mod tests {
    ...
    /// For a real model's tensor list, the type frink chooses per
    ...
    #[test]
    fn frink_chooses_the_same_type_llama_quantize_chose_for_every_tensor() {
        let rows = rows();
            ...
            for r in &rows {
                ...
                match (&r.chosen, allowed) {
                    ...
                    (Some(_), Some(reason)) => panic!(
                        "{}: llama-quantize quantized it, frink keeps it at source precision \
                         ({reason})",
                        r.name
                    ),
                    (None, None) => panic!(
                        "{}: llama-quantize kept it at source precision, frink would quantize it",
                        r.name
                    ),
                }
            }
