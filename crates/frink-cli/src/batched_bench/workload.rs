...
//! `frink bench` applies to its own rows.
//!
...
//! per row and buys the determinism assertion `frink bench` has: a
...
pub(super) fn measure(decoder: &Decoder, combo: Combo, shape: &Shape) -> anyhow::Result<Measured> {
    ...
    // checked BEFORE the clock, like every stream `frink bench` feeds.
    ...
    for rep in 0..1 + bench_guard::WARMUP_REPS {
        ...
        // prompt in one batch and lets `n_ubatch` slice it; frink has
        ...
        // unlike `frink bench` the host cache is always the record and
