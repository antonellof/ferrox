...
//! (header) and `:245` (row), reproduced width for width so a frink
//! table and a llama.cpp table paste side by side. The JSONL line is
//! `:238-243` minus the keys that would describe things frink does
...
/// `:126`, with frink's values and without the three fields that
/// have no frink meaning.
...
/// `:238-243`, with the same per-row keys and the header keys frink
/// can answer for.
...
mod tests {
    ...
    /// written for `llama-batched-bench` output reads frink output.
    #[test]
    fn jsonl_rows_carry_upstreams_per_row_keys() {
        ...
        // What frink has no value for is absent, not invented.
