...
fn child_tokens(
    ...
    // `FRINK_METAL` alone only turns on the Metal *matvecs*: the fused
    // attention block, its RoPE kernels and the resident KV are behind
    // `FRINK_METAL_ATTN`, which `run --ngl 99` sets and this harness
    ...
    // pre-set value so `FRINK_METAL_ATTN=0 frink verify …` still
    // ablates.
