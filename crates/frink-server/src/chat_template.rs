...
//! There is one now (`frink_models::chat_template`, which compiles the
...
use frink_models::chat_template::{
...
///
/// `frink_models`'s own `handles_tools` is a substring check on the
