...
//!
//! * A **decoder** (`FRINK_MODEL_PATH`). Its final hidden states get
...
//!
//! Both go through [`frink_models::pooling::pool`]. That matters: the
...
use frink_models::pooling::{l2_normalize, pool, PoolingType};
...
/// [`frink_models::pooling::pool`] itself.
            ...
            if active_model.embed_tokens(&[]).is_none() {
                return Err(unsupported_feature(
                    "embeddings engine not yet available for this model. Load a real \
                     ...
                     the served model (FRINK_MODEL_PATH), or beside this one \
