...
//! `vocab_size` floats. `frink-cli` and `frink-server` each decide
//! this per request and announce it with [`set_metal_greedy_argmax`];
//! `frink-models` reads it back with [`metal_greedy_argmax_active`] and
...
//!
//! Fixed in `frink-models` (GitHub issue #170), where that gate lives.
...
//! by looking at what other threads chose. It cannot. `frink-server`'s
...
//!
//! `frink_core::par::carry`, which is the one thing in the workspace
...
///
/// The seam a work-stealing scheduler needs: `frink_core::par`'s
