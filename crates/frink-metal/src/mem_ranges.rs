...
//! ```
//!
//! Frink tracks whole MTLBuffer identities (llama uses byte ranges on the
...
//!
//! Set `FRINK_METAL_BARRIER_LOG=1` to log the running barriers-per-op
...
fn barrier_log_enabled() -> bool {
    ...
    *ON.get_or_init(|| std::env::var_os("FRINK_METAL_BARRIER_LOG").is_some())
}
...
impl MemRanges {
    ...
    /// encoded straight into the concurrent pass. `FRINK_METAL_GPU_TIMING`
        ...
        if !self.check(srcs, dsts) {
            ...
            if barrier_log_enabled() {
                ...
                if s.barriers.is_multiple_of(4096) {
                    eprintln!(
                        "frink: metal barriers={} begin_ops={} (~{:.2} bar/op)",
                        s.barriers,
                        s.begin_ops,
