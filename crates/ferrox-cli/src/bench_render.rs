//! `ferrox bench --render`: turn the receipts on disk into the engine
...
pub const BEGIN: &str = "<!-- BEGIN ENGINE TABLE (generated by `ferrox bench --render`) -->";
...
struct ModelRow {
    model: String,
    pp_test: String,
    pp_ferrox: Option<f64>,
    pp_llama: Option<f64>,
    pp_gap: Option<f64>,
    tg_test: String,
    tg_ferrox: Option<f64>,
    tg_llama: Option<f64>,
    tg_gap: Option<f64>,
}
...
pub fn render(bench_dir: &Path) -> anyhow::Result<()> {
    ...
    fn push_section_at(table: &mut String, depth: &str, title: &str, rows: &[&Row]) {
        ...
        table.push_str(&format!(
            "| Model | Prefill | Decode | ferrox {pp} | llama.cpp {pp} | ferrox {tg} | llama.cpp {tg} |\n"
        ));
        table.push_str("|---|---|---|---:|---:|---:|---:|\n");
        for m in &models {
            table.push_str(&format!(
    ...
    table.push_str(
        "---\n\n\
         Generated by `ferrox bench --render` from [`receipts/engine/`](receipts/engine/). \
...
mod tests {
    ...
    fn receipt_with(dir: &Path, id: &str, host: &str, backend: &str, tests: &[(&str, f64, f64)]) {
        let tests: Vec<serde_json::Value> = tests
            .iter()
            .map(|(test, ferrox, llama)| {
                serde_json::json!({
                    "test": test, "ferrox_tps": ferrox, "llama_tps": llama,
                    "gap": llama / ferrox,
                })
            })
            .collect();
    ...
    fn receipt(dir: &Path, id: &str, host: &str, backend: &str, ferrox: f64, llama: f64) {
        receipt_with(dir, id, host, backend, &[("tg128", ferrox, llama)]);
    }
    ...
    ///
    /// Sabotage: drop the `e.tg_ferrox = r.ferrox` assignment in
    ...
    fn the_column_headers_name_the_workload_that_ran() {
        ...
        render(&dir).expect("render");
        let out = std::fs::read_to_string(dir.join("RESULTS.md")).expect("read");
        assert!(out.contains("ferrox pp2048"), "header not derived:\n{out}");
    ...
    /// silently stop mentioning a model ferrox runs.
    ///
