...
//! model, and `frink_models::ModelConfig::layer_ffn_act(il)` is the
...
/// `frink-models` hardcoded SwiGLU while only the dense arm consulted
...
/// GELU approximation (`frink_core::matmul::gelu`), which is what
...
/// with nothing enforcing it. See `frink_core::matmul::reglu`.
///
...
/// [`Swiglu`](GluAct::Swiglu), and `frink_models::act_layers` is what
...
impl GluAct {
    ...
    pub fn apply(self, gate: &[f32], up: &[f32]) -> Vec<f32> {
        match self {
            ...
            GluAct::Reglu => frink_core::matmul::reglu(gate, up),
            GluAct::ReluSqr => relu_sqr(up),
            GluAct::GeluUngated => up.iter().map(|&x| frink_core::matmul::gelu(x)).collect(),
    ...
    pub fn combine(self, gate: f32, up: f32) -> f32 {
        match self {
            GluAct::Swiglu => frink_core::matmul::silu(gate) * up,
            GluAct::SwigluClamped { limit, form } => {
                let g = match form {
                    ClampForm::AfterSilu => frink_core::matmul::silu(gate).min(limit),
                    ClampForm::BeforeSilu => frink_core::matmul::silu(gate.min(limit)),
                };
                g * up.clamp(-limit, limit)
            }
            GluAct::Geglu => frink_core::matmul::gelu(gate) * up,
            GluAct::Reglu => frink_core::matmul::relu(gate) * up,
            GluAct::ReluSqr => {
                let r = frink_core::matmul::relu(up);
                r * r
            }
            GluAct::GeluUngated => frink_core::matmul::gelu(up),
...
impl Ungated {
    ...
    pub fn apply(self, up: &[f32]) -> Vec<f32> {
        match self {
            ...
            Ungated::Gelu => up.iter().map(|&x| frink_core::matmul::gelu(x)).collect(),
...
pub fn relu_sqr(up: &[f32]) -> Vec<f32> {
    up.iter()
        .map(|&x| {
            let r = frink_core::matmul::relu(x);
            r * r
        })
        .collect()
}
...
mod tests {
    ...
    fn clamped_swiglu_matches_llama_cpp_s_else_branch() {
        ...
        let silu = frink_core::matmul::silu;
        // Both clamps bite.
