...
//! the test that pins them against `ferrox_models::gdn`'s host body.
...
// `ferrox_core::mamba2::conv_step` followed by SiLU, one thread per
// convolution channel.
//
...
// `ferrox_core::gdn::l2_normalize` over one head of Q or K: one
...
kernel void gdn_gates(
    ...
    // `ferrox_core::mamba2::softplus`, threshold and all: above 20 the
...
/// `ferrox_models::gdn::Gdn::conv_and_gates_for_rows` does for one row.
