...
//! twins are held to against `frink_quant`. Run
//! `cargo test -p frink-cuda --features cuda -- --ignored` on a device
...
/// nibble element `j + 16`.
///
/// Mirrors `frink_quant::dot_iq4_nl_f32_scalar` except that the scale
...
__constant__ float frink_kvalues_iq4nl[16] = {
...
extern "C" __global__ void iq4_nl_matvec(
    ...
    for (int b = threadIdx.x; b < n_blocks_per_row; b += blockDim.x) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            block_acc += frink_kvalues_iq4nl[byte & 0x0F] * x[base + i];
            block_acc += frink_kvalues_iq4nl[byte >> 4] * x[base + i + 16];
        }
        acc += block_acc * scale;
    }
...
/// `frink_quant::dot_iq4_xs_f32_scalar`, with the per-group scale
    ...
    for (int b = threadIdx.x; b < n_blocks_per_row; b += blockDim.x) {
        ...
        for (int ib = 0; ib < 8; ib++) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                group_acc += frink_kvalues_iq4nl[byte & 0x0F] * x[gbase + i];
                group_acc += frink_kvalues_iq4nl[byte >> 4] * x[gbase + i + 16];
            }
            acc += group_acc * dl;
        }
    }
...
/// `2^(e-127)` scale, following `frink_quant`; ggml stores the values
...
///
/// Mirrors `frink_quant::dot_mxfp4_gguf_f32_scalar`, with the scale
/// factored out of the inner loop.
pub const MXFP4_MATVEC_KERNEL_SRC: &str = r#"
__constant__ float frink_kvalues_mxfp4[16] = {
...
extern "C" __global__ void mxfp4_matvec(
    ...
    for (int b = threadIdx.x; b < n_blocks_per_row; b += blockDim.x) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            block_acc += frink_kvalues_mxfp4[byte & 0x0F] * x[base + i];
            block_acc += frink_kvalues_mxfp4[byte >> 4] * x[base + i + 16];
        }
        acc += block_acc * scale;
    }
