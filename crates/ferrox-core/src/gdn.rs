...
mod tests {
    ...
    /// The batched device entry is the same recurrence run row by row:
    /// one state buffer, one submission, the rows in order. Compared
    /// against the host stepping the same rows, because "sequential in
    /// rows" is exactly what a batched kernel gets wrong.
    #[cfg(feature = "metal")]
    #[test]
    #[ignore = "needs a real Metal-capable GPU; run manually with --ignored on Apple Silicon"]
    fn the_device_delta_rows_step_in_order() {
        use ferrox_metal::gdn::{DeltaShape, HeadMapKind};

        let (n_k, n_v, s, rows) = (2usize, 8usize, 32usize, 5usize);
        let dims = DeltaDims {
            n_k_heads: n_k,
            n_v_heads: n_v,
            head_dim: s,
        ...
        let state0 = draw(dims.state_len(), 0.3);
        ...
        for r in 0..rows {
            ...
            host_out[r * n_v * s..(r + 1) * n_v * s].copy_from_slice(&o);
        }

        let mut device_state = state0.clone();
        let device_out = ferrox_metal::gdn::launch_delta_step_rows(
            DeltaShape {
                n_k_heads: n_k,
                n_v_heads: n_v,
                head_dim: s,
        ...
        .expect("the batched kernel launches");

        for (i, (a, b)) in device_out.iter().zip(host_out.iter()).enumerate() {
            assert!(
                (a - b).abs() <= 2e-4 * b.abs().max(1.0),
                "row {} out[{i}]: device={a} host={b}",
                i / (n_v * s)
            );
        }
        for (i, (a, b)) in device_state.iter().zip(host_state.iter()).enumerate() {
