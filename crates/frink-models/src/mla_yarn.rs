...
//! bands (`frink_core::attention::yarn_freq_factors`, the same
...
use frink_core::attention::yarn_freq_factors;
        ...
        return Err(LoadError::UnsupportedFeature(
            ...
            format!(
                "`{arch}.rope.scaling.type = yarn` without \
                 ...
                 converter writes the key (conversion/base.py:1235) and frink does not guess it"
            ),
        ));
    }
