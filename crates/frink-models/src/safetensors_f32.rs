...
//! through `frink_quant`'s widening functions. Anything else is `None`
...
pub(crate) fn widen_to_f32(dtype: SafetensorsDtype, raw: &[u8]) -> Option<Vec<f32>> {
    match dtype {
        ...
        SafetensorsDtype::F16 => frink_quant::dequant_f16(raw).ok(),
        SafetensorsDtype::BF16 => frink_quant::dequant_bf16(raw).ok(),
