//! Loads `frink-models::glm52_decoder` weights from a GLM-5.2 GGUF
//! checkpoint, via `frink_gguf::TensorSource` -- the same trait
//! `frink-models::loader`'s generic GQA path and
...
//! `frink_models::mla` already has via `causal_mla_attention_sparse`,
...
/// `frink_moe::ExpertWeights`'s field names/order.
pub struct Glm52MoeFfnWeights {
    pub router_weight: WeightMatrix,
    pub e_score_correction_bias: Vec<f32>,
    pub experts: Vec<frink_moe::ExpertWeights>,
    pub shared_expert: frink_moe::ExpertWeights,
}
...
pub fn load_glm52_moe_ffn(
    ...
    let experts = gate_exps
        ...
        .zip(down_exps)
        .zip(up_exps)
        .map(|((gate, down), up)| frink_moe::ExpertWeights { gate, up, down })
        .collect();

    let shared_expert = frink_moe::ExpertWeights {
...
fn load_embedding_tensor(
    file: &impl TensorSource,
    hidden_dim: usize,
) -> Result<frink_core::tensor::Tensor, LoadError> {
    ...
    Ok(frink_core::tensor::Tensor::new(
        data,
...
mod tests {
    ...
    fn loads_a_full_indexer_dense_layer_and_a_shared_indexer_moe_layer() {
        ...
        let path =
            std::env::temp_dir().join(format!("frink_glm52_gguf_test_{}.gguf", std::process::id()));
    ...
    fn wk_b_transpose_matches_hand_computed_values() {
        ...
        let bytes = build_gguf("glm-dsa", &tensors);
        let path =
            std::env::temp_dir().join(format!("frink_glm52_wk_b_test_{}.gguf", std::process::id()));
        std::fs::write(&path, &bytes).unwrap();
        let file = frink_gguf::GgufFile::open(&path).expect("synthetic GGUF must parse");
