...
//!   (`:219-238`, `server-context.cpp:1752-1800`). frink does not
...
//! projections. Shapes here are `[rows, cols]` in frink's row-major
//! sense, i.e. ggml's `ne` reversed.

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};

use frink_gguf::{GgufFile, TensorSource};
...
pub enum LoraError {
    ...
    #[error(
        "{path}: this is an activated LoRA (`adapter.alora.invocation_tokens`, {n} tokens), \
         ...
         (server-context.cpp:1752-1800); frink applies an adapter to every position and \
    ...
    #[error(
        "{path}: '{name}' adapts a routed-expert tensor; llama.cpp applies that through \
         `build_lora_mm_id` (llama-graph.cpp:1517-1550) and frink has no per-expert delta, \
         so the adapter is refused rather than applied to the dense projections only"
    )]
    RoutedExperts { path: PathBuf, name: String },
    #[error(
        "{path}: '{name}' adapts a tensor frink holds under no projection (norms, biases and \
...
impl LoraAdapter {
    ...
    fn from_file(path: PathBuf, file: &GgufFile) -> Result<Self, LoraError> {
        ...
        if let Some(v) = file.metadata("adapter.alora.invocation_tokens") {
            let n = match v {
                frink_gguf::GgufValue::Array(items) => items.len(),
                _ => 1,
            };
...
mod tests {
    use super::*;
    use frink_gguf::writer::{GgufWriter, TensorPlan};
    ...
    fn write_adapter(
        name: &str,
        kv: &[(&str, frink_gguf::GgufValue)],
        tensors: &[(&str, Vec<u64>)],
    ) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("frink-lora-{}-{name}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("adapter.gguf");
        let metadata: BTreeMap<String, frink_gguf::GgufValue> =
    ...
    fn base_kv() -> Vec<(&'static str, frink_gguf::GgufValue)> {
    ...
    fn a_control_vector_adapter_is_refused_by_type() {
        ...
        kv[1] = (
            "adapter.type",
            frink_gguf::GgufValue::String("control_vector".into()),
        );
    ...
    fn an_activated_lora_is_refused_by_name() {
        ...
        kv.push((
            ...
            frink_gguf::GgufValue::Array(vec![
                frink_gguf::GgufValue::U32(5),
                frink_gguf::GgufValue::U32(9),
            ]),
        ));
