...
impl ScoredVocab {
    ...
    pub(crate) fn from_gguf(
        ...
        let scores: Vec<f32> = match file.metadata("tokenizer.ggml.scores") {
            Some(frink_gguf::GgufValue::Array(items)) => {
...
pub(crate) struct MetadataOnlyGguf {
    meta: HashMap<String, frink_gguf::GgufValue>,
}
...
impl MetadataOnlyGguf {
    ...
    pub(crate) fn with(mut self, key: &str, value: frink_gguf::GgufValue) -> Self {
    ...
    pub(crate) fn with_tokens(self, tokens: &[&str]) -> Self {
        let items = tokens
            .iter()
            .map(|t| frink_gguf::GgufValue::String((*t).to_string()))
            .collect();
        self.with("tokenizer.ggml.tokens", frink_gguf::GgufValue::Array(items))
    }

    pub(crate) fn with_scores(self, scores: &[f32]) -> Self {
        let items = scores
            .iter()
            .map(|s| frink_gguf::GgufValue::F32(*s))
            .collect();
        self.with("tokenizer.ggml.scores", frink_gguf::GgufValue::Array(items))
    }
}

#[cfg(test)]
impl frink_gguf::TensorSource for MetadataOnlyGguf {
    fn metadata(&self, key: &str) -> Option<&frink_gguf::GgufValue> {
        self.meta.get(key)
    }
    fn find_tensor(&self, _name: &str) -> Option<&frink_gguf::TensorInfo> {
        None
    }
