...
//! default sampler chain, on top of the greedy argmax frink previously
...
///
/// frink keeps its `argmax` fast path, because building and sorting a
...
/// [`tests::the_default_order_is_the_chain_frink_already_ran`].
///
...
/// of steps frink implements, there is no arm here that means "asked
...
mod tests {
    ...
    fn the_chain_frink_used_to_hardcode(
        logits: &[f32],
        params: &SamplingParams,
        history: PenaltyWindow<'_>,
    ) -> Vec<f32> {
    ...
    /// and this goes red.
    #[test]
    fn the_default_order_is_the_chain_frink_already_ran() {
        ...
        for (temperature, top_k, top_p, min_p) in [
            ...
            let expected = the_chain_frink_used_to_hardcode(&logits, &params, window());
            ...
            for (i, (a, e)) in actual.iter().zip(expected.iter()).enumerate() {
                assert_eq!(
                    ...
                    "token {i} at temp {temperature}, top_k {top_k}, top_p {top_p}, \
                     min_p {min_p}: the default order sampled {a} where the chain frink \
                     already ran gives {e}"
                );
            }
        }
    ...
    /// `tools/completion/completion.cpp:730-736`); frink's decode
    ...
    /// (`common/common.h:259-269`); frink divided first and filtered
    ...
    /// (`common/common.h:231`), so until this existed frink could not
    ...
    fn min_p_truncates_at_ln_p_below_the_top_logit() {
        ...
        // 0.0 disables it, which is frink's struct default -- adding
    ...
    /// and frink's greedy fast path knows that.
    ///
