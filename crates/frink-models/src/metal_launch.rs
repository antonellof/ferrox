...
//! `frink_metal::gpu::matvec_launch_meta` and not here shows up as a
...
pub(crate) fn matvec<'a>(m: &'a WeightMatrix) -> Option<frink_metal::gpu::MatvecLaunch<'a>> {
    match m {
        WeightMatrix::F32(t) => {
            let rows = t.shape[0];
            let cols = t.shape[1];
            let (src, fn_name, block_bytes, block_elems, rows_per_tg) =
                frink_metal::gpu::matvec_launch_meta("F32")?;
        ...
        WeightMatrix::Quantized {
            ...
            // `frink_metal::gpu::MATVEC_KINDS` served both.
