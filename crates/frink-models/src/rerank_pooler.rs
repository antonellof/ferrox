//! `frink splice-pooler`: put a `BertForSequenceClassification`
...
//!    re-convert. Not landed upstream, and not something frink controls.
...
//! (`frink-server::slots::identity`) is not reused here on purpose: it
...
//! [`POOLER_SOURCE_KEY`] naming the safetensors file so `frink inspect`
...
/// visible in `frink inspect` and a second splice refuses by name.
