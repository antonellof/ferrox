...
/// picked differently.
///
/// ONCE EACH is the whole subtlety, and frink used to get it wrong.
...
/// times is divided by `penalty_repeat` exactly once. frink walked the
/// HISTORY, so the same token was divided `n` times and the effective
/// penalty was `penalty^n`.
///
/// That was live on every `frink run`: `--repeat-penalty` defaults to
...
mod tests {
    ...
    ///
    /// frink walked the history and divided once per OCCURRENCE, so the
    ...
    ///
    /// Live on every `frink run`: `--repeat-penalty` defaults to 1.1,
    ...
    /// `common/common.h:238`); frink scanned everything generated so
