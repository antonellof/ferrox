...
impl SamplingParams {
    ...
    /// dominant defect: `frink_cli::run`'s `needs_vocab_logits`,
    /// `frink_server::generate`'s, and through them the Metal fold's
    /// own guard in `frink_metal::greedy_fold`.
    ///
...
mod tests {
    ...
    /// in. Built here rather than imported because `frink-models` does
    /// not depend on `frink-cli`. `frink_cli::run::tests::
    ...
    /// so does every gate test in `frink-cli` and `frink-server`.
    #[test]
