...
//! response cache destructures one EXHAUSTIVELY (`frink_server::
...
//! frink's CLI flags, where the person who typed them can see them, and
...
/// deterministic behavior frink always had before this module existed.
#[derive(Debug, Clone)]
pub struct SamplingParams {
    ...
    /// so without it frink could not reproduce llama.cpp's *own*
    ...
    /// is frink's "do nothing the caller did not ask for" baseline, and
    ...
    ///
    /// `0` disables the penalties entirely. frink had no window at all
...
impl SamplingParams {
    ...
    /// one function because frink reads it in two places too: the RNG
...
mod tests {
    ...
    /// chain be frink's default chain without changing any existing
    /// run's output.
    ///
