...
//! from `frink-models::decoder::Decoder` (which every GQA-only preset
//! -- GLM-5.2, DeepSeek V4 Pro, the test fixtures -- uses and which
//! `frink-cli`, `frink-server`, `prefix_cache`, and `speculative` all
//! depend on): this module composes the already-independently-tested
//! `kda`, `mla`, `latent_moe`, `block_residual`, and
//! `frink_core::situ_and_mul` pieces into a real forward pass, without
