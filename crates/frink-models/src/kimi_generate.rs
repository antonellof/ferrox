...
//! `frink-server::generate`'s loop (encode prompt, decode incrementally,
...
//! API rather than `frink_core::cache::KvCache`.
...
/// `finish_reason`/token counts the way `frink-server` does for its
/// GGUF path).
...
pub fn kimi_generate(
    ...
    frink_core::par::on_workers(move || {
        generate_on_workers(
            weights,
            cfg,
            mla_cfg,
...
/// point of this path: `frink run-kimi` calls `kimi_generate` and
...
mod tests {
    ...
    /// once per parallel region of every token. `frink run-kimi` is
    /// the caller.
    ///
    ...
    fn a_whole_kimi_generation_enters_the_pool_once() {
        ...
        let count = |f: &dyn Fn()| {
            let before = frink_core::par::cold_regions();
            f();
            frink_core::par::cold_regions() - before
        };
        let raw = count(&|| {
            generate_on_workers(
                &weights,
