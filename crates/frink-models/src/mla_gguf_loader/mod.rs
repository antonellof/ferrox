...
pub fn read_deepseek2_hparams(file: &impl TensorSource) -> Result<Deepseek2Hparams, LoadError> {
    ...
    let n_vocab = file
        ...
        .or_else(|| match file.metadata("tokenizer.ggml.tokens") {
            Some(frink_gguf::GgufValue::Array(items)) => Some(items.len()),
            _ => None,
        })
        .unwrap_or(0);
    ...
    // spellings are still accepted after both, because frink's own
...
fn load_mla_attn(
    ...
    let kv_b = match (
        ...
        (false, false, false) => {
            return Err(LoadError::Gguf(frink_gguf::GgufError::TensorNotFound(
...
fn load_dense_ffn(
    ...
    Ok(MlaDenseFfn {
        weights: frink_moe::ExpertWeights {
            gate,
...
fn load_moe_ffn(
    ...
    let experts = gate_exps
        ...
        .zip(up_exps)
        .zip(down_exps)
        .map(|((gate, up), down)| frink_moe::ExpertWeights { gate, up, down })
        .collect();
