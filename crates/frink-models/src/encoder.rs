...
pub enum EncodeError {
    ...
    #[error(
        "layer {layer} runs a gated FFN and carries no ffn_gate; the loader builds the \
         pair together, so this is a bug in frink rather than in the checkpoint"
    )]
...
pub trait TextEncoder: Sync {
    ...
    ///
    /// This is on the encoder rather than the tokenizer because frink's
    ...
    ///
    /// Do not override. See
    /// [`frink_core::par::on_workers`] for the three cases it declines
    /// to promote.
    fn encode(&self, tokens: &[u32], segments: Option<&[u32]>) -> Result<Vec<f32>, EncodeError> {
        frink_core::par::on_workers(move || self.encode_on_worker(tokens, segments))
    }
