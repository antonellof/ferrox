...
//!    `frink_core::attention::apply_rope_interleaved`) — the opposite
...
//!    `frink_core::attention::causal_mla_attention` rather than
...
//! dispatch to this yet) or into `frink_core::cache::KvCache` (which
...
/// The two forms compute the same attention (`frink_core::
...
/// alone -- plain `Vec<f32>`, not yet `frink_core::cache::KvCache`
...
/// with YaRN's `mscale^2` folded in (`frink_core::mla_absorbed`,
