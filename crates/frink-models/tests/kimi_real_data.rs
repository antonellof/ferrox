//! Verifies `frink-models::kimi_loader`/`kimi_decoder` against a real,
...
//! `frink-cuda`'s real-GPU test uses for hardware it can't assume is
//! present.
//!
...
//!    ```
//! 2. `export FRINK_TEST_KIMI_SHARD_DIR=/path/to/that/directory`
//! 3. `cargo test -p frink-models --test kimi_real_data -- --ignored --nocapture`
//!
...
fn loads_and_runs_a_real_kimi_k3_layer_from_downloaded_bytes() {
    let dir = std::env::var("FRINK_TEST_KIMI_SHARD_DIR")
        .expect("set FRINK_TEST_KIMI_SHARD_DIR to the directory containing the downloaded shard");
    ...
    index_json.push_str("}}");
    let index_path = shard_dir.join("frink_test_index.safetensors.index.json");
    ...
    let kda_weights = frink_models::kimi_loader::load_kda_attn(
        &shard,
        prefix,
        KDA_NUM_HEADS,
        KDA_HEAD_DIM,
    ...
    let dense_weights =
        frink_models::kimi_loader::load_dense_mlp(&shard, prefix, HIDDEN_DIM, DENSE_INTERMEDIATE)
            .expect("must load real dense MLP weights");
    let block_res = frink_models::kimi_loader::load_block_residual(&shard, prefix)
        .expect("must load real block-residual weights");
    let input_layernorm_weight = frink_models::kimi_loader::load_f32_vec(
        &shard,
        &format!("{prefix}.input_layernorm.weight"),
    )
    .expect("must load real input_layernorm weight");
    let post_attention_layernorm_weight = frink_models::kimi_loader::load_f32_vec(
        &shard,
    ...
    let layer = frink_models::kimi_decoder::KimiDecoderLayerWeights {
        input_layernorm_weight,
        attn: frink_models::kimi_decoder::KimiLayerAttention::Kda(Box::new(kda_weights)),
        post_attention_layernorm_weight,
        ffn: frink_models::kimi_decoder::KimiLayerFfn::Dense(Box::new(dense_weights)),
    ...
    let weights = frink_models::kimi_decoder::KimiDecoderWeights {
        embedding: frink_core::tensor::Tensor::new(embedding_row, vec![1, HIDDEN_DIM]),
        layers: vec![layer],
        output_attn_res_norm_weight,
        output_attn_res_proj_weight,
        final_norm_weight,
        output_head: frink_core::weight_matrix::WeightMatrix::F32(frink_core::tensor::Tensor::new(
            output_head_data,
            vec![output_vocab, HIDDEN_DIM],
        )),
    };

    let cfg = frink_models::kimi_decoder::KimiDecoderConfig {
    ...
    let mla_cfg = frink_models::config::MlaConfig {
        num_heads: 96,
        q_lora_rank: 1536,
        kv_lora_rank: 512,
    ...
    let kda_cfg = frink_models::config::KdaConfig {
    ...
    let mut state = frink_models::kimi_decoder::KimiDecodeState::new(&weights, &kda_cfg);
    let logits = frink_models::kimi_decoder::kimi_forward_token(
...
///    ```
/// 2. `export FRINK_TEST_KIMI_MOE_SHARD_DIR=/path/to/that/directory`
/// 3. `cargo test -p frink-models --test kimi_real_data moe_layer -- --ignored --nocapture`
#[test]
#[ignore]
fn loads_and_runs_a_real_kimi_k3_moe_layer_from_downloaded_bytes() {
    let dir = std::env::var("FRINK_TEST_KIMI_MOE_SHARD_DIR").expect(
        "set FRINK_TEST_KIMI_MOE_SHARD_DIR to the directory containing the downloaded shard",
    );
    let shard_dir = std::path::PathBuf::from(dir);
    ...
    let index_path = shard_dir.join("frink_test_moe_index.safetensors.index.json");
    ...
    let layer = frink_models::kimi_loader::load_kimi_layer(
        &shard,
        &hp,
        frink_models::config::LayerAttentionKind::KimiKda,
        false,
        layer_idx,
    )
    ...
    match &layer.ffn {
        frink_models::kimi_decoder::KimiLayerFfn::Moe(moe) => {
            assert_eq!(moe.experts.n_experts(), hp.n_experts);
        }
        frink_models::kimi_decoder::KimiLayerFfn::Dense(_) => panic!("expected a real MoE FFN"),
    }
    assert!(matches!(
        layer.attn,
        frink_models::kimi_decoder::KimiLayerAttention::Kda(_)
    ));
...
fn loads_real_kda_attention_and_a_sample_of_real_mxfp4_experts_for_layer_1() {
    ...
    let index_json = format!("{{\"weight_map\":{{{}}}}}", entries.join(","));
    let index_path = shard_dir.join("frink_test_sample_index.safetensors.index.json");
    ...
    for &e in &sample_expert_indices {
        let expert = frink_models::kimi_loader::load_kimi_expert(
            &shard,
...
/// Verifies `frink-models::kimi_tokenizer::KimiTokenizer` against a
...
/// 2. `export FRINK_TEST_KIMI_TOKENIZER_PATH=/path/to/tiktoken.model`
/// 3. `cargo test -p frink-models --test kimi_real_data tokenizer -- --ignored --nocapture`
#[test]
#[ignore]
fn kimi_tokenizer_matches_the_real_tiktoken_library_on_real_vocab() {
    let path = std::env::var("FRINK_TEST_KIMI_TOKENIZER_PATH")
        .expect("set FRINK_TEST_KIMI_TOKENIZER_PATH to the downloaded tiktoken.model file");
    let text = std::fs::read_to_string(&path).expect("must read tiktoken.model");
    let ranks = frink_models::kimi_tokenizer::parse_tiktoken_vocab(&text)
    ...
    let tok =
        frink_models::kimi_tokenizer::KimiTokenizer::new(ranks, std::collections::HashMap::new())
    ...
    for (text, expected) in cases {
        let ids = tok.encode(text, frink_models::tokenizer::SpecialTokens::AsText);
