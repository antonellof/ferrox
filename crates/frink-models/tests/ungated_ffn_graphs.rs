...
//! (`frink_models::mla_arch`, tests/plm_graphs.rs).
//!
...
//! `frink_moe::GluAct::ReluSqr` (`relu(up)^2`, reading `up` alone) and
...
//!     crates/frink-models/tests/fixtures/arcee_tiny.gguf
//! /tmp/ref_logits crates/frink-models/tests/fixtures/arcee_tiny.gguf 3 7 11 19 23 5
//! ```
...
/// tensors; expected 21, got 19` (measured). frink's refusal is
...
fn a_file_with_a_gate_is_refused_like_llama_cpp_refuses_it() {
    ...
    let msg = match frink_models::Decoder::from_gguf(&path, config) {
