//! `frink splice-pooler` end to end: a GGUF goes in without its
...
//! cargo test -p frink-models --test rerank_pooler_splice -- --ignored --nocapture
//! ```

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};

use frink_gguf::{GgmlType, GgufFile, GgufValue, GgufWriter, ShardedGguf, TensorPlan};
use frink_models::rerank_pooler::POOLER_SOURCE_KEY;
use frink_models::{load_rank_head, splice_pooler, EmbeddingModel, SpliceError};
    ...
    snapshot.unwrap_or_else(|| {
        panic!(
            "neither {} nor a cached snapshot under {} exists; this test fails rather than \
             skips.\n    frink download cross-encoder/ms-marco-MiniLM-L6-v2 model.safetensors \
...
fn the_spliced_real_checkpoint_matches_huggingface_on_all_four_query_sets() {
    ...
    for (query, documents, want) in HF_REFERENCE {
        ...
        assert_eq!(
            ranking(&scores),
            ranking(want),
            "{query}: frink {scores:?}, HuggingFace {want:?}"
        );
        for (i, (got, want)) in scores.iter().zip(want).enumerate() {
            ...
            assert!(
                ...
                "{query}, document {i}: frink {got}, HuggingFace {want}"
            );
        }
    }
    eprintln!("largest |frink - HuggingFace| across 17 pairs: {worst}");
