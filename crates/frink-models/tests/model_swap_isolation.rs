...
//!
//! Requires:
//! - `cargo test -p frink-models --features metal --test model_swap_isolation -- --ignored --nocapture`
//! - Apple Silicon + Metal (`FRINK_METAL=0` ablates it to the CPU path)
...
const SEQUENCE_VAR: &str = "FRINK_TEST_SWAP_SEQUENCE";
...
#[test]
#[ignore = "needs Apple Metal GPU + the GGUFs under models/ (FRINK_TEST_MODELS_DIR to relocate)"]
fn a_checkpoint_answers_the_same_after_another_checkpoint_was_loaded_first() {
    ...
    if frink_metal::gpu::probe().is_none() && std::env::var("FRINK_METAL").as_deref() != Ok("0") {
