...
//!
//! Requires:
//! - `cargo test -p frink-models --features metal --test smollm2_metal_paris -- --ignored`
//! - Apple Silicon + Metal
//! - `models/hf_test/SmolLM2-135M-Instruct-Q8_0.gguf` (or `FRINK_TEST_SMOLLM2_GGUF`)
//!
//! Mirrors the llama.cpp parity check documented in `frink-moe` (Paris vs
...
fn smollm2_gguf_path() -> PathBuf {
    if let Ok(p) = std::env::var("FRINK_TEST_SMOLLM2_GGUF") {
...
#[test]
#[ignore = "needs Apple Metal GPU + models/hf_test/SmolLM2-135M-Instruct-Q8_0.gguf (or FRINK_TEST_SMOLLM2_GGUF)"]
