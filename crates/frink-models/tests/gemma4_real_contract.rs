...
//! logits for it and `frink parity` skips it. What is checked here is
//! the contract — that every key frink requires exists, that every
//! value frink derives matches the derivation in
...
//!
//! Run: `cargo test -p frink-models --test gemma4_real_contract -- --ignored`

use std::path::{Path, PathBuf};

use frink_gguf::{GgufFile, TensorSource};
use frink_models::engine::Engine;
use frink_models::gemma4_gguf_loader::{load_gemma4_engine, read_gemma4_hparams};
use frink_models::tokenizer::{should_add_bos_token, GgufBpeTokenizer, SpecialTokens};

const PROMPT: &str = "The capital of France is";
const MAX_NEW_TOKENS: usize = 8;

fn gemma4_path() -> PathBuf {
    if let Ok(p) = std::env::var("FRINK_TEST_GEMMA4_GGUF") {
...
/// Every value frink derives matches llama.cpp's own derivation for
