...
//! `cargo test -p frink-models --test streaming_bytes -- --ignored`
use frink_models::{config::ModelConfig, Decoder};
...
fn a_streamed_expert_sees_the_same_bytes_as_a_resident_one() {
    ...
    let file = frink_gguf::ShardedGguf::open(MODEL).expect("open");
...
fn streamed_and_resident_agree_on_a_real_checkpoint() {
    ...
    for prompt in [
        ...
        let ids_of = |d: &Decoder| -> Vec<usize> {
            let mut caches: Vec<frink_core::cache::KvCache> = d.config.new_kv_caches();
