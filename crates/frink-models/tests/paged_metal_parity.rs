...
//!
//! `frink verify` compares CPU against Metal over the CONTIGUOUS cache
...
//!
//! Requires:
//! - `cargo test -p frink-models --features metal --test paged_metal_parity -- --ignored --nocapture`
//! - Apple Silicon + Metal
//! - the GGUFs under `models/` (`FRINK_TEST_MODELS_DIR` to point elsewhere),
//!   or `FRINK_TEST_PAGED_PARITY_GGUF` for a single file
...
#[test]
#[ignore = "needs Apple Metal GPU + the GGUFs under models/ (or FRINK_TEST_PAGED_PARITY_GGUF)"]
fn paged_kv_answers_exactly_what_contiguous_kv_answers_on_metal() {
    ...
    // Honour a pre-set value so `FRINK_METAL=0 cargo test …` ablates to
    // the CPU pair, which is how the two halves of this bug were told
    // apart in the first place.
    for (k, v) in [("FRINK_METAL", "1"), ("FRINK_METAL_ATTN", "1")] {
        if std::env::var(k).is_err() {
            std::env::set_var(k, v);
        }
    }

    match std::env::var("FRINK_TEST_PAGED_PARITY_GGUF") {
...
/// holding.
///
/// The re-invocation is the shape `frink verify` already uses for the
...
fn check_each_in_its_own_process() {
    ...
    for name in MODELS {
        ...
        let out = std::process::Command::new(&exe)
            ...
            .env("FRINK_TEST_PAGED_PARITY_GGUF", &path)
            .output()
