...
//! Run:
//! ```text
//! export FRINK_TEST_RECEIPT_CHECKPOINT=/path/to/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf
//! cargo test -p frink-models --test checkpoint_receipts -- --ignored --nocapture
//! ```
...
#[test]
#[ignore = "needs FRINK_TEST_RECEIPT_CHECKPOINT pointing at the pinned Llama-3.1 Q4_K_M GGUF"]
fn llama31_8b_q4km_forced_continuation_receipt() {
    let receipt = load_receipt("llama31_8b_instruct_q4_k_m.json");
    let path = std::env::var("FRINK_TEST_RECEIPT_CHECKPOINT").unwrap_or_else(|_| {
        panic!(
            "set FRINK_TEST_RECEIPT_CHECKPOINT to the local path of {}",
