...
//! `libllama` build), not by re-reading a spec and not by frink
...
//!     crates/frink-models/tests/fixtures/gptoss_tiny.gguf
...
//! /tmp/gptoss_ref crates/frink-models/tests/fixtures/gptoss_tiny.gguf 3 7 11 19 23 5
//! ```
...
/// The claim: frink's gpt-oss CPU graph agrees with llama.cpp's.
#[test]
...
fn gpt_oss_loader_wires_the_whole_graph() {
    ...
    // graph that creates the tensor (`frink_models::proj_bias`).
    for layer in &decoder.layers {
        assert_eq!(
    ...
    assert_eq!(
        ...
        frink_models::swa_layers::SwaLayers::period(2, false)
    );
    ...
    assert_eq!(
        ...
        frink_models::config::RopeLayout::Neox
    );
...
fn gpt_oss_golden_is_not_vacuous() {
    ...
    // 1. No attention sinks (the term frink had nowhere at all).
    {
    ...
    // 5. Sliding window applied to every layer (what frink did before
    //    `default_swa_pattern`: a missing pattern key meant "all SWA").
    {
        let mut decoder = load();
        decoder.config.swa_layers = frink_models::swa_layers::SwaLayers::All;
        let mut caches: Vec<KvCache> = decoder.config.new_kv_caches();
        let broken = decoder.forward_batch_last(&PROMPT, 0, &mut caches);
        assert!(
            max_delta(&broken) > TOL * 10.0,
    ...
    {
        let mut decoder = load();
        decoder.config.rope_layout = frink_models::config::RopeLayout::Norm;
        let mut caches: Vec<KvCache> = decoder.config.new_kv_caches();
        let broken = decoder.forward_batch_last(&PROMPT, 0, &mut caches);
        assert!(
            max_delta(&broken) > TOL * 10.0,
