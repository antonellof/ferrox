...
//!    for it are byte-identical to the file without it. frink refuses
...
//! and not by frink checking itself.
//!
//! Regenerating (all three files and both goldens together):
//!
//! ```text
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_minicpm_fixture.py \
//!     crates/frink-models/tests/fixtures/minicpm_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_minicpm_fixture.py \
//!     crates/frink-models/tests/fixtures/minicpm_declared_tiny.gguf --declared
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_minicpm_fixture.py \
//!     crates/frink-models/tests/fixtures/minicpm_attention_scale_tiny.gguf \
//!     --attention-scale
...
//! /tmp/ref_logits crates/frink-models/tests/fixtures/minicpm_tiny.gguf 3 7 11 19 23 5
//! ```
...
/// The file declares no key, so a frink that ignored `MultiplierDefaults`
...
/// architecture uses, so this is a fact frink carries in a table, and a
...
/// `f_attention_scale` from anything. frink stops instead of honouring
...
fn the_derived_refusal_list_holds_only_the_keys_minicpm_does_not_read() {
    let keys: Vec<String> = frink_models::capability::unsupported_scaling_keys("minicpm")
        .into_iter()
        .map(|(k, _, _)| k)
        .collect();
    assert_eq!(
