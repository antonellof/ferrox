...
//!
//! `frink_core::par`'s helpers funnel into `rayon::join` and the
...
/// the weights.
const FIXTURE: &str = "tests/fixtures/frink_real_test.gguf";

fn load() -> Decoder {
    let file = frink_gguf::GgufFile::open(FIXTURE).expect("fixture opens");
...
///
/// `FRINK_CPU_POOL=spin` deliberately keeps the rayon pool unbuilt, a
...
/// to `frink-core`, so the copy was the only way to ask. It is not: an
...
#[test]
#[ignore = "needs models/gemma-4-E2B-it-Q4_K_M.gguf (or FRINK_TEST_GEMMA4_GGUF)"]
fn a_gemma4_decode_step_enters_the_pool_exactly_once() {
    ...
    let path = std::env::var("FRINK_TEST_GEMMA4_GGUF").unwrap_or_else(|_| {
    ...
    let file = frink_gguf::GgufFile::open(&path).expect("open GGUF");
    let engine = frink_models::gemma4_gguf_loader::load_gemma4_engine(&file).expect("load");
...
#[test]
#[ignore = "needs models/bge-small-en-v1.5-q8_0.gguf (or FRINK_TEST_BERT_GGUF)"]
fn one_encoder_pass_enters_the_pool_exactly_once() {
    use frink_models::TextEncoder;

    if !the_promotion_applies_here() {
        return;
    }
    let path = std::env::var("FRINK_TEST_BERT_GGUF").unwrap_or_else(|_| {
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
    ...
    let encoder = frink_models::load_bert_encoder_from_path(&path).expect("load");
