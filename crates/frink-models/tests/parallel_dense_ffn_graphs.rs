...
//! `frink_models::parallel_dense_ffn` (presence, sum scale), served
...
//! | `arctic_wscale` | the same file declaring `expert_weights_scale = 2.5`, which `arctic.cpp:3-14` never reads; libllama's logits are BYTE-IDENTICAL to `arctic`'s (measured), so the golden is shared and the test pins that frink ignores the key too |
...
//! measured by dropping it). frink applies it where upstream does;
//! the test pins the measurement rather than pretending to a
//! sabotage.
//!
//! ```text
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_arctic_fixture.py \
//!     crates/frink-models/tests/fixtures/arctic_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_arctic_fixture.py \
//!     crates/frink-models/tests/fixtures/arctic_wscale_tiny.gguf --weights-scale 2.5
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_grok_fixture.py \
//!     crates/frink-models/tests/fixtures/grok_dense_ffn_tiny.gguf --dense-ffn
...
use frink_models::parallel_dense_ffn::{parallel_dense_ffn, DensePresence};
use frink_models::router_input::RouterInput;
...
/// (measured), so the golden is the same array and frink must not be
/// the one that honours it (`EXPERT_WEIGHTS_SCALE_READERS`).
#[test]
fn arctic_ignores_expert_weights_scale_as_llama_cpp_does() {
    let file = frink_gguf::GgufFile::open(common::graph_fixture_path(ARCTIC_WSCALE)).unwrap();
    assert_eq!(
