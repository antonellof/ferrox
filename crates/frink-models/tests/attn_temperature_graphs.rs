...
//! `log(floor(pos / floor_scale) + 1) * temp_scale + 1`. frink had no
...
//!    job is to adjust YaRN's MAGNITUDE term, and frink turned out not
...
//!     crates/frink-models/tests/fixtures/mistral3_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_mistral3_fixture.py \
//!     crates/frink-models/tests/fixtures/mistral3_temp_tiny.gguf --temp
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_mistral3_fixture.py \
//!     crates/frink-models/tests/fixtures/mistral3_temp_ctx_tiny.gguf --temp-ctx
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_mistral3_fixture.py \
//!     crates/frink-models/tests/fixtures/mistral3_yarn_tiny.gguf --yarn
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_mistral3_fixture.py \
//!     crates/frink-models/tests/fixtures/mistral3_yarn_logmul_tiny.gguf --yarn-logmul
...
use frink_models::attn_temperature::AttnTemperature;
use frink_models::{ModelConfig, RopeLayout};
...
fn a_temperature_with_no_floor_is_refused_like_llama_cpp_refuses_it() {
    ...
    let file = frink_gguf::GgufFile::open(graph_fixture_path(TEMP_CTX)).expect("fixture opens");
    ...
    let ok = frink_gguf::GgufFile::open(graph_fixture_path(TEMP_CTX)).expect("fixture opens");
