...
//! life of this project (see `frink parity`'s tokenizer module). So the
//! assertion here is against llama.cpp's ids, token for token.
//!
//! # The two tests, and why there are two
//!
//! * [`frink_matches_the_recorded_llama_cpp_tokenization`] needs only
...
//!   frozen numbers from becoming a record of frink agreeing with
//!   itself.
//!
...
//! cargo test -p frink-models --test wordpiece_llama_cpp_parity -- --ignored
//! ```
//!
//! The recorded ids came from llama.cpp at
//! `1269cb1ff1598751f846241be90083ae9ad036fb`, which is the revision in
//! `.scratch/llama.cpp` that [`frink_models::GgufWordPieceTokenizer`]
...
//! reporting a frink defect. Upstream's WordPiece normalizer grew
...
//! `BertNormalizer` drops it, and so does the revision above, so frink
...
//! # What the corpus is
//!
//! The first nineteen cases are `frink parity`'s own corpus, verbatim,
...
//! shared because the corpus lives in a private module of `frink-cli`,
...
use frink_models::GgufWordPieceTokenizer;
...
/// The reference dumper, overridable with `FRINK_LLAMA_LOGITS`.
///
...
fn dumper_path() -> PathBuf {
    match std::env::var_os("FRINK_LLAMA_LOGITS") {
...
fn tokenizer() -> Option<GgufWordPieceTokenizer> {
    ...
    assert!(
        frink_models::tokenizer::should_add_bos_token(&file),
        "llama.cpp's WPM arm sets add_bos = true, so frink must too"
    );
...
fn report(
    ...
    format!(
        "[{name}] diverges at token {at} of {} (llama) / {} (frink)\n  \
         text   {text:?}\n  llama  {}\n  frink {}",
        llama.len(),
        frink.len(),
        window(llama),
...
/// The evidence. Frink's ids, against llama.cpp's, on every case.
#[test]
#[ignore = "needs models/bge-small-en-v1.5-q8_0.gguf"]
fn frink_matches_the_recorded_llama_cpp_tokenization() {
    ...
    for case in GOLDEN {
        let frink = tok.encode(case.text, SpecialTokens::Parse);
        tokens += case.llama.len();
        if frink != case.llama {
            failures.push(report(&tok, case.name, case.text, case.llama, &frink));
        }
    }
...
///
/// Without this the frozen ids would decay into a record of frink
/// agreeing with frink.
///
...
/// same call `frink parity` makes when llama.cpp cannot load a
...
fn the_recorded_tokenization_is_still_what_llama_cpp_produces() {
    ...
    assert!(
        ...
        "the recorded table is not what this libllama produces, and the \
         ...
         a real change. \"llama\" is the live answer, \"frink\" is the \
         recorded one.\n\n{}",
        failures.join("\n\n")
    );
}
...
fn reference_tokenize(dumper: &Path, model: &Path, texts: &[&str]) -> (u32, usize, Vec<Vec<u32>>) {
    ...
    let in_path = dir.join(format!("frink-wpm-cases-{pid}.bin"));
    let out_path = dir.join(format!("frink-wpm-toks-{pid}.bin"));
