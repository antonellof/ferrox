...
//!
//! ```text
//! FRINK_LORA_MODEL=models/Llama-3.2-1B-Instruct-Q8_0.gguf
//! FRINK_LORA_ADAPTERS=adapter.gguf:1.0[,other.gguf:0.5]
//! FRINK_LORA_TOKENS="128000 791 6864 315 9822 374"   # "The capital of France is"
//! FRINK_LORA_GOLDEN=/tmp/ref.txt   # `ref_logits --lora adapter.gguf:1.0 model.gguf 1 504 ...`
//! cargo test -p frink-models --test lora_real_checkpoint -- --nocapture
//! ```
//!
//! `FRINK_LORA_ADAPTERS=` (empty) measures the base alone, which is
...
fn a_real_checkpoint_with_an_adapter_matches_libllama() {
    let Ok(model) = std::env::var("FRINK_LORA_MODEL") else {
        eprintln!("FRINK_LORA_MODEL unset; skipping");
        return;
    };
    let golden_path = std::env::var("FRINK_LORA_GOLDEN").expect("FRINK_LORA_GOLDEN");
    let tokens: Vec<usize> = std::env::var("FRINK_LORA_TOKENS")
        .expect("FRINK_LORA_TOKENS")
        .split_whitespace()
        .map(|t| t.parse().expect("token id"))
        .collect();
    let adapters = std::env::var("FRINK_LORA_ADAPTERS").unwrap_or_default();
    ...
    // `frink parity`'s MATCH line (`parity.rs` `KL_NOISE`): below it the
