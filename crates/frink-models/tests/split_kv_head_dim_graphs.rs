...
//! Every KV cache, attention kernel and projection check in frink
//! took ONE head width, and the loader refused the file.
//! `frink_models::kv_head_dims` is the seam and has the census:
...
//! export): `frink_models::attn_value_scale`.
//!
...
//! Not by re-reading a spec, and not by frink checking itself.
//!
...
//!     crates/frink-models/tests/fixtures/mimo2_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_mimo2_fixture.py \
//!     crates/frink-models/tests/fixtures/mimo2_split_tiny.gguf --split
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_mimo2_fixture.py \
//!     crates/frink-models/tests/fixtures/mimo2_novscale_tiny.gguf --no-value-scale
...
fn the_loader_resolves_the_row_the_way_llama_cpp_does() {
    ...
    // golden) and so must frink: `EXPERT_WEIGHTS_SCALE_READERS`. This
...
fn a_split_width_on_an_architecture_that_asserts_them_equal_is_refused() {
    let err = frink_models::kv_head_dims::resolve_v_head_dim("qwen3moe", 192, Some(128))
...
/// two goldens differ by construction, and this pins that frink
...
/// key in the file and is unscaled). Honouring it -- what frink did
