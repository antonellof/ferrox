...
//! with a NULL `ffn_gate_inp`. Every other MoE body in frink routed on
...
//! (`frink_models::router_input` has the table): four of 155 graphs
...
//! spec, and not by frink checking itself.
//!
...
//!     crates/frink-models/tests/fixtures/smallthinker_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_smallthinker_fixture.py \
//!     crates/frink-models/tests/fixtures/smallthinker_nowindow_tiny.gguf --no-window
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_smallthinker_fixture.py \
//!     crates/frink-models/tests/fixtures/smallthinker_swa2_tiny.gguf --swa-period 2
...
use frink_models::capability::{swa_window_override, SwaWindowOverride};
use frink_models::rope_layers::{NoRopePhase, RopeLayers};
use frink_models::router_input::RouterInput;
use frink_models::FfnActivation;
...
fn routing_on_the_normed_input_picks_different_experts_and_diverges_from_llama_cpp() {
    ...
    let counts = |d: &frink_models::Decoder| -> Vec<Vec<u64>> {
        d.layers
            .iter()
            .map(|l| {
                l.moe
...
fn the_swa_period_comes_from_the_key_not_the_nope_step() {
    ...
    d.config.swa_layers = frink_models::swa_layers::SwaLayers::Period {
