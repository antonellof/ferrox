...
//! 1/sqrt(n_embd_head_k)`), which frink had only inside `MlaEngine`,
...
//! (`frink_models::mla_arch`): a direct `attn_q` (`plm.cpp:32`;
//! `frink_models::mla_q_proj`, which the lite DeepSeek-V2 checkpoints
...
//! | `plm_decoy_output` | the same file with an `output.weight` the graph does not read; libllama REFUSES it (`done_getting_tensors: wrong number of tensors; expected 30, got 29`, measured), so frink must too rather than prefer the decoy |
//!
...
//!     crates/frink-models/tests/fixtures/plm_tiny.gguf
//! PYTHONPATH=$LLAMA/gguf-py python3 scripts/make_plm_fixture.py \
//!     crates/frink-models/tests/fixtures/plm_decoy_output_tiny.gguf --decoy-output
//! /tmp/ref_logits crates/frink-models/tests/fixtures/plm_tiny.gguf 3 7 11 19 23 5
//! ```

mod common;
use common::{assert_close, graph_fixture_path, kl_vs_golden, worst_vs, GRAPH_PROMPT, GRAPH_TOL};
use frink_models::engine::Engine;
use frink_models::mla::MlaQProj;
use frink_models::mla_arch::{mla_arch, MlaOutputHead, QProjRule};
use frink_models::{load_mla_engine_from_path, LoadError, MlaEngine, ServedEngine};
...
fn plm_matches_llama_cpp() {
    ...
    println!(
        "plm: KL(llama.cpp || frink) = {:.3e}, max |delta| = {:.3e}",
...
fn the_loaded_engine_is_the_plm_row() {
    ...
    for layer in &engine.layers {
        ...
        match &layer.ffn {
            frink_models::MlaLayerFfn::Dense(d) => {
                assert!(matches!(d.act, GluAct::ReluSqr));
                assert_eq!(d.weights.up.rows(), 48);
            }
            frink_models::MlaLayerFfn::Moe(_) => panic!("plm is dense"),
        }
    }
...
fn each_of_the_rows_decisions_is_visible_in_the_logits() {
    ...
    for layer in engine.layers.iter_mut() {
        if let frink_models::MlaLayerFfn::Dense(d) = &mut layer.ffn {
            d.act = GluAct::Swiglu;
        }
    }
    assert!(
    ...
    engine.mla_cfg.rope = Some(frink_models::config::MlaRopeConfig { theta: 500.0 });
    assert!(
        worst_vs(&decode(&engine), &PLM_GOLDEN) > 1e-2,
        "rope theta not seen"
    );
