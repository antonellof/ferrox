...
//! runs on, and a Metal decode step is not that: `frink-cli` and
//! `frink-server` each decide per request whether the stack may fold
...
//! # Settings, not caches
//!
//! Only state a CALLER configured belongs here. `frink-metal`'s other
...
//! the buffer it describes (`frink_metal::resident_act`).
//!
...
pub(crate) struct Carry {
    ...
    /// `frink_metal::greedy_fold`.
    #[cfg(feature = "metal")]
    greedy_fold: frink_metal::greedy_fold::GreedyFold,
}
...
impl Carry {
    ...
    pub(crate) fn capture() -> Self {
        Self {
            #[cfg(feature = "metal")]
            greedy_fold: frink_metal::greedy_fold::greedy_fold_setting(),
        }
    }
    ...
    pub(crate) fn adopt(self) -> Adopted {
        ...
        Adopted {
            #[cfg(feature = "metal")]
            _greedy_fold: frink_metal::greedy_fold::adopt_greedy_fold(greedy_fold),
        }
    }
}
...
pub(crate) struct Adopted {
    #[cfg(feature = "metal")]
    _greedy_fold: frink_metal::greedy_fold::GreedyFoldGuard,
}
...
///   remaining `frink-metal` thread-locals are caches, mirrors and
///   scratch (this module's header lists them). Verified bit-identical
///   before and after with `frink verify` and `frink parity` on a
...
/// - **`Cuda`** -- audited clean and NOT promoted. `frink-cuda` has no
...
mod tests {
    ...
    ///
    /// The `frink-metal` half of this is tested at its source; what is
    ...
    fn an_adopted_setting_does_not_outlive_its_job() {
        use frink_metal::greedy_fold::{greedy_fold_setting, set_metal_greedy_argmax, GreedyFold};
