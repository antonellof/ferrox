//! CPU worker-pool policy, shared by `frink` (CLI) and `frink-server`.
//!
...
//!    use -- which inside `frink-server` is a Tokio `spawn_blocking`
...
pub fn set_user_interactive_qos() {
    #[cfg(target_os = "macos")]
    {
        ...
        if std::env::var_os("FRINK_QOS_LOG").is_some() {
            eprintln!(
                "frink: worker {:?} qos {} -> {} (rc={rc})",
...
/// How many rayon workers to run: `FRINK_CPU_THREADS`, else
/// `RAYON_NUM_THREADS`, else [`perf_core_count`].
pub fn resolve_cpu_threads() -> usize {
    for key in ["FRINK_CPU_THREADS", "RAYON_NUM_THREADS"] {
...
mod tests {
    ...
    fn resolved_thread_count_falls_back_to_perf_cores_without_env_overrides() {
        ...
        if std::env::var_os("FRINK_CPU_THREADS").is_none()
