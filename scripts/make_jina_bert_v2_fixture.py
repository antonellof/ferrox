#!/usr/bin/env python3
"""Generate the tiny synthetic `jina-bert-v2` GGUF.
...
def main(out_path: str) -> None:
    ...
    w.add_name("ferrox-jina-bert-v2-fixture")
    ...
    for il in range(N_LAYER):
        ...
        # The QK LayerNorm pair (optional upstream, present here) and
        # the second attention norm, both with biases.
        w.add_tensor(p + "attn_q_norm.weight", rnd(N_HEAD * HEAD_DIM) + 1.0)
        w.add_tensor(p + "attn_q_norm.bias", rnd(N_HEAD * HEAD_DIM))
        w.add_tensor(p + "attn_k_norm.weight", rnd(N_HEAD * HEAD_DIM) + 1.0)
        w.add_tensor(p + "attn_k_norm.bias", rnd(N_HEAD * HEAD_DIM))
        w.add_tensor(p + "attn_norm_2.weight", rnd(N_EMBD) + 1.0)
        w.add_tensor(p + "attn_norm_2.bias", rnd(N_EMBD))
        # GEGLU with the gate FUSED into ffn_up: 2 * n_ff rows, the
        # first half the gate (bert.cpp:189, `up_contains_gate`).
