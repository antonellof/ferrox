#!/usr/bin/env python3
"""Generate the tiny synthetic `neo-bert` GGUF (pre-norm RMS encoder).
...
def main(out_path: str) -> None:
    ...
    w.add_name("ferrox-neo-bert-fixture")
    ...
    w.add_layer_norm_rms_eps(LAYER_NORM_EPS)
    ...
    w.add_tensor("enc.output_norm.weight", rnd(N_EMBD) + 1.0)

    for il in range(N_LAYER):
        p = f"blk.{il}."
        w.add_tensor(p + "attn_norm.weight", rnd(N_EMBD) + 1.0)
        w.add_tensor(p + "ffn_norm.weight", rnd(N_EMBD) + 1.0)
        w.add_tensor(p + "attn_qkv.weight", rnd(3 * N_HEAD * HEAD_DIM, N_EMBD))
